...
async function run() {
    const prompt = \`
Analyze the following 300-line poem. We are building a procedural animation engine that will visualize this poem using a "Petroglyph Dictionary"—a visual alphabet made of primitive, child-like geometric symbols (like a stick figure, a sun with rays, wavy lines for water, a triangle for a house, jagged lines for energy).

Based purely on the events, nouns, and verbs in this poem, generate exactly what our complete "Petroglyph Dictionary" needs to be to tell this entire story from beginning to end. 

Group them into logical categories (e.g., Nature, Human/Entities, Objects/Technology, Abstract/Action).
For each suggested petroglyph, provide:
1. The Name (e.g., "The Ship")
2. The Geometry (e.g., "A semi-circle bottom with a tall triangle sail")
3. When it is used in the poem.

Make sure you cover the entire arc: from the Taino origins, the Spanish arrival, the slavery era, Trujillo and the '65 war, the modern 80s/90s chaos, the "cancer" of modern society, and the futuristic return to nature/Cacicazgos. Keep it to around 15-20 highly reusable, core symbols.

Here is the poem:
\${poem}
\`;

    console.log("Analyzing the 300-line poem with Gemini...");
    const result = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
        contents: prompt
    });

    fs.writeFileSync('G-3.1-DICTIONARY.md', result.text);
    console.log("Dictionary successfully generated to G-3.1-DICTIONARY.md!");
}
